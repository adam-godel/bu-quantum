...
requires: entanglement-and-bell-states
summary: 'Protocols that use entanglement to do things classical communication cannot.'
---
<KatexSpan>
## Table of Contents
...
## Pseudo-Telepathy
This page is one of my favorites, since we are starting to look at some actual quantum algorithms, in particular ones that leverage entanglement to achieve some really cool results. 
...
Once Bob then receives Alice's qubit, he simply undos the entanglement by performing a $CX$ gate with Alice's qubit as the control and his qubit as the target and then an $H$ gate on Alice's qubit. This will collapse the two qubits into the bitstring Alice encoded with certainty.
