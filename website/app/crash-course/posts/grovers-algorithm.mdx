...
requires: deutsch-jozsa-and-simons-algorithms
summary: 'Searching an unstructured space quadratically faster than any classical search.'
---
<KatexSpan>
...
You should have an intuitive sense of how this problem is structured, but let's word it in a way that a quantum computer would understand. Specifically, the key intuition is to formulate it similarly to the oracular problems covered earlier.
...
<m.Mafs viewBox={{x: [-0.25, 1], y: [-1, 1], padding: 0.3}} pan={false}>
    <m.Coordinates.Cartesian 
        xAxis={{lines: 0.5, labels: (n) => Math.abs(n) == 1 ? (n == -1 ? "" : "|A₀⟩") : ""}} 
        yAxis={{lines: 0.5, labels: (n) => Math.abs(n) == 1 ? (n == -1 ? "" : "|A₁⟩") : ""}} 
        subdivisions={4}
    />
    ...
    <m.Text x={pt0.x} y={pt0.y} attach="e" attachDistance={15} size={20}>|ψ⟩</m.Text>
    <m.Text x={pt0.x} y={-pt0.y} attach="e" attachDistance={5} size={20}>𝑂|ψ⟩</m.Text>
    <m.Text x={Math.cos(Math.atan2(pt0.y, pt0.x) + 2*Math.asin(0.25))} y={Math.sin(Math.atan2(pt0.y, pt0.x) + 2*Math.asin(0.25))} attach="e" attachDistance={5} size={20}>𝐺|ψ⟩</m.Text>
    <m.Plot.Parametric
...
\end{cases}
$$`}
both with and without using the built-in Grover operator functions in Qiskit or Classiq (for those who have read [Bernstein-Vazirani Algorithm, Intro to Classiq](/crash-course/bernstein-vazirani-and-classiq)). An oracle like this (but at a much larger scale) is a bit more "practical" since the oracle's test is far less obvious from just looking at each bitstring.
...
<a name="foot2">2</a>. I feel like this is much easier to "see" and understand visually than it is to compute. If you're interested in computing this rotation rigorously and a discussion of the generalization of Grover's algorithm, known as **amplitude amplification**, see [Rotation Matrices, Amplitude Amplification](/crash-course/amplitude-amplification).
</KatexSpan>