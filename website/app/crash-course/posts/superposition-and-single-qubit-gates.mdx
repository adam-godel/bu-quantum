...
requires: qubits-and-the-bloch-sphere, bra-ket-notation
summary: 'A formal definition of superposition, the single-qubit gates, and your first circuit in Qiskit.'
---
<KatexSpan>
## Table of Contents
...
## Superposition
Let's generalize the definition of superposition introduced earlier. For two perfectly distinguishable quantum states $|A\rangle$ and $|B\rangle$ and two complex numbers $\alpha,\beta$ where $|\alpha|^2+|\beta|^2=1$, a **superposition** of $|A\rangle$ and $|B\rangle$ is defined as
$$
...
A **single-qubit quantum gate** is a $2 \times 2$ [unitary matrix](https://en.wikipedia.org/wiki/Unitary_matrix). All of the quantum gates we will be looking at are also **Hermitian**, which means that they are equal to their conjugate transpose. This means that for a quantum gate $U$, $U^2=I$, so applying the gate a second time will reverse it.
...
<Quiz 
    question={String.raw`$HY|0\rangle=XHY|0\rangle$.`}
    answers={[
        "True",
        "False",
    ]}
    correct={0}
    explanation={[
        <span>We know that <InlineMath>Y|0\rangle=i|1\rangle</InlineMath>, which is a global phase, so this is equivalent to <InlineMath>|1\rangle</InlineMath>. From there, the <InlineMath>H</InlineMath> gate will make this state <InlineMath>|-\rangle</InlineMath>, and we know that the <InlineMath>X</InlineMath> gate does not affect the <InlineMath>|-\rangle</InlineMath> state.</span>, 
        <span>We know that <InlineMath>Y|0\rangle=i|1\rangle</InlineMath>, which is a global phase, so this is equivalent to <InlineMath>|1\rangle</InlineMath>. From there, the <InlineMath>H</InlineMath> gate will make this state <InlineMath>|-\rangle</InlineMath>, and we know that the <InlineMath>X</InlineMath> gate does not affect the <InlineMath>|-\rangle</InlineMath> state.</span>,
    ]}
/>
...
This is what the Qiskit Aer library is for. It allows us to create a local quantum simulator where we can measure the state, go back in time, and remeasure as many times as we want.<sup><a href="#foot2">2</a></sup> Qiskit calls this local simulator a sampler, and each instance we measure our circuit using the sampler is known as a shot.
```
...
<a name="foot2">2</a>. We will not be using actual quantum hardware here, simply because every quantum backend (as of the time I'm writing this) requires you to either pay or enter a credit card for verification, and I don't want that to be a barrier for anyone. Also, running a circuit on actual quantum hardware usually takes at least a few hours. If you have a credit card for verification and you'd like to execute your circuits on quantum hardware, see [Using Quantum Hardware](/crash-course/using-quantum-hardware).
</KatexSpan>