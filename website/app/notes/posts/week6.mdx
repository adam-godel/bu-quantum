...
{String.raw`$$
t = \left\lfloor \frac{\pi}{4\theta} \right\rfloor = \left\lfloor \frac{\pi}{4\arcsin(\sqrt{M/N})} \right\rfloor \approx \left\lfloor \frac{\pi}{4}\sqrt{\frac{N}{M}} \right\rfloor
$$`}
and since each Grover operator takes $O(1)$ time to apply, we are left with a total time complexity of $O(\sqrt\{N/M\})$ = $O(\sqrt\{N\})$.
