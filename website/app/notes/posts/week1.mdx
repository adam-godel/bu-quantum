...
Now, I am not saying that in a few decades, quantum computing will change the world as much as classical computing will—[quantum hype](https://scottaaronson.blog/?p=8329) is a very real problem! However, quantum computing let us create new types of computers that throw out our prior assumptions about what algorithms can be run efficiently.
...
$$`}
In quantum mechanics, $e^\{i\varphi\}$ is called the relative phase. More intuitively, for those who are familiar with [spherical coordinates](https://en.wikipedia.org/wiki/Spherical_coordinate_system), the state $|\psi\rangle$ is simply mapped onto the point $(1,\theta,\varphi)$. More specifically, we draw $|\psi\rangle$ as a unit vector from the origin to that point on the Bloch sphere.
...
<a name="foot1">1</a>. This state is written in [bra-ket notation](https://en.wikipedia.org/wiki/Bra%E2%80%93ket_notation), which is widely used in quantum physics and computing. Bra-ket notation can be pretty confusing to understand at first, so I recommend checking out this week's [addendum](/notes/week1-addendum) to learn more about it.
