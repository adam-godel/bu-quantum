...
Below is an example of possible quantum state if we restrained them to be only real numbers, so we can represent it in a 2D grid. Feel free to play around with it to get a good feel for the different possible states.
...
<m.Mafs viewBox={{x: [-1, 1], y: [-1, 1], padding: 0.25}} pan={false}>
    <m.Coordinates.Cartesian 
        xAxis={{lines: 0.5, labels: (n) => Math.abs(n) == 1 ? (n == -1 ? "−" : "")+"|1〉" : ""}} 
        yAxis={{lines: 0.5, labels: (n) => Math.abs(n) == 1 ? (n == -1 ? "−" : "")+"|0〉" : ""}} 
        subdivisions={4}
    />
    <m.Circle
        center={[0, 0]}
